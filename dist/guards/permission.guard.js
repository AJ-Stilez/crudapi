...
let PermissionGuard = class PermissionGuard {
    ...
    canActivate(context) {
        ...
        if (!user || !user.permissions) {
            throw new common_1.ForbiddenException('You do not have the permission to access this page');
        }
        const hasPermission = required.every((permission) => user.permissions.includes(permission));
        if (!hasPermission) {
            throw new common_1.ForbiddenException('You do not have enough permission entry to access this page');
        }
        return true;
    }
};
